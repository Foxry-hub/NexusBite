...
type CategoryType = "all" | "best-seller" | string;
...
export default function SiswaDashboardClient({ initialUser }: { initialUser: { id: string; name: string; email: string; role: string; balance: number } }) {
  ...
  // Get unique sellers for category filter
  const sellers = useMemo(() => {
    const sellerMap = new Map<string, { id: string; name: string }>();
    menus.forEach((menu) => {
      if (menu.seller && !sellerMap.has(menu.seller.id)) {
        sellerMap.set(menu.seller.id, menu.seller);
      }
    });
    return Array.from(sellerMap.values());
  }, [menus]);

  // Best sellers (menus with most orders)
  ...
  const displayMenus = useMemo(() => {
    if (activeCategory === "all") {
      ...
      // Filter by seller
      return availableMenus.filter((m) => m.seller?.id === activeCategory);
    }
  }, [availableMenus, activeCategory, bestSellers]);
    ...
    <div className="min-h-screen bg-neutral-950">
      ...
      <main className="ml-64 min-h-screen">
        ...
        <div className="p-8">
          ...
          <div className="mb-8 animate-fade-in-up animation-delay-300">
            <div className="flex items-center gap-3 overflow-x-auto pb-2 scrollbar-hide">
              ...
              {sellers.map((seller) => (
                <button
                  key={seller.id}
                  onClick={() => setActiveCategory(seller.id)}
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium text-sm whitespace-nowrap transition-all duration-300 ${
                    activeCategory === seller.id
                      ? "bg-orange-500 text-white shadow-lg shadow-orange-500/30"
                      : "bg-neutral-800/50 text-neutral-400 hover:bg-neutral-800 hover:text-white"
                  }`}
                >
          ...
          {/* Best Seller Section - Only show on "all" view */}
          {activeCategory === "all" && bestSellers.length > 0 && !searchQuery && (
            <div className="mb-10 animate-fade-in-up animation-delay-300">
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-white">Best Seller 🔥</h2>
                    <p className="text-neutral-500 text-sm">Menu paling laris minggu ini</p>
                  </div>
                </div>
                <button
                  onClick={() => setActiveCategory("best-seller")}
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                {bestSellers.slice(0, 6).map((menu, index) => {
                  const cartItem = mounted ? items.find((i) => i.menuId === menu.id) : null;
                  return (
                    <div
                      key={menu.id}
                      className="bg-gradient-to-br from-neutral-900 to-neutral-900/50 border border-orange-500/20 rounded-2xl overflow-hidden hover:border-orange-500/50 transition-all duration-300 hover-lift animate-pop-in group"
                      style={{ animationDelay: `${0.1 + index * 0.05}s` }}
                    >
                      <div className="relative h-28 bg-gradient-to-br from-neutral-800 to-neutral-900 flex items-center justify-center overflow-hidden">
                        {menu.image ? (
                          <img 
                            src={menu.image} 
                            alt={menu.name} 
                          ...
                          <Utensils className="w-10 h-10 text-neutral-700 group-hover:text-orange-500/50 transition-colors duration-300" />
                        )}
                        {/* Best Seller Badge */}
                        <div className="absolute top-2 left-2 bg-gradient-to-r from-orange-500 to-red-500 px-2 py-1 rounded-lg text-[10px] font-bold text-white flex items-center gap-1">
                          <TrendingUp className="w-3 h-3" />
                          #{index + 1}
                        </div>
                        {/* Order count */}
                        <div className="absolute bottom-2 right-2 bg-black/60 backdrop-blur-sm px-2 py-1 rounded-lg text-[10px] text-white">
                          {menu.totalOrders}x terjual
                        </div>
                      </div>
                      <div className="p-3">
                        <h3 className="font-semibold text-white text-xs mb-1 line-clamp-1">{menu.name}</h3>
                          ...
                          {cartItem ? (
                            <div className="flex items-center gap-1">
                              <button
                                ...
                                className="w-6 h-6 rounded-md bg-neutral-800 text-white flex items-center justify-center hover:bg-neutral-700 transition-colors"
                              >
                                <Minus className="w-3 h-3" />
                              </button>
                              <span className="text-white text-xs w-4 text-center font-medium">{cartItem.quantity}</span>
                              <button
                                onClick={() => updateQuantity(menu.id, cartItem.quantity + 1)}
                            ...
                            <button
                              ...
                              className="w-7 h-7 rounded-lg bg-orange-500 text-white flex items-center justify-center hover:bg-orange-600 transition-all duration-300 hover:scale-110"
                            >
              ...
              <div>
                <h2 className="text-xl font-bold text-white">
                  {activeCategory === "best-seller" 
                    ? "Best Seller" 
                    : activeCategory !== "all" 
                      ? sellers.find(s => s.id === activeCategory)?.name || "Menu"
                      : "Menu Tersedia"}
                </h2>
                <p className="text-neutral-500 text-sm">{displayMenus.length} menu {activeCategory === "best-seller" ? "terlaris" : "siap dipesan"}</p>
              </div>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-16 animate-fade-in-up">
                ...
                <p className="text-neutral-500 text-sm">
                  {searchQuery ? "Coba kata kunci lain" : "Coba cek lagi nanti ya"}
                </p>
                {activeCategory !== "all" && (
                  <button
                    onClick={() => setActiveCategory("all")}
                  ...
                  return (
                    ...
                    >
                      <div className="relative h-40 bg-gradient-to-br from-neutral-800 to-neutral-900 flex items-center justify-center overflow-hidden">
                        ...
                        {/* Show rank badge for best sellers */}
                        {activeCategory === "best-seller" && (
                          <div className="absolute top-2 left-2 bg-gradient-to-r from-orange-500 to-red-500 px-2 py-1 rounded-lg text-xs font-bold text-white flex items-center gap-1">
                            <TrendingUp className="w-3 h-3" />
                            #{index + 1}
                          </div>
                        )}
          ...
          {/* Unavailable Menus - Only show on "all" category */}
          {activeCategory === "all" && unavailableMenus.length > 0 && (
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center gap-1 text-xs text-neutral-600 mb-1">
                        <Store className="w-3 h-3" />
                        <span>{menu.seller?.name || 'Penjual'}</span>
                      </div>
        ...
        {mounted && getTotalItems() > 0 && !showCart && (
          <button
            onClick={() => setShowCart(true)}
